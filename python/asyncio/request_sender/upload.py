...
class Uploader(object):
    ...
    async def run(self):
        while True:
            ...
            endpoint = f"{self.base_url}/v1/{route}"
...
async def rate_limited_queue(ns1objs: List[str], q: queue.Queue, rate: int, fname: str) -> None:
    """
...
if __name__ == "__main__":
    ...
    # Tasks and await primitives must be inside of a async def
    fill_queue_task = asyncio.create_task(rate_limited_queue(objects, work_q, rate, args.file))

    sender_tasks = []
    for i in range(num_workers):
        # TODO: Test if this actually works
        uploader = Uploader(args.apikey, args.verify, f"https://{args.api_host}", work_q)
        sender_task = asyncio.create_task(uploader.run())
        sender_tasks.append(sender_task)
        logging.debug(f"Starting working task: {i}")
